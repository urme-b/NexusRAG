...
from nexusrag.agents import LLMClient, Orchestrator, RAGResponse
...
class NexusRAG:
    ...
    def ingest_bytes(self, data: bytes, filename: str, extension: str) -> IngestResult:
        ...
        try:
            ...
            if document.id in self.document_store.index:
                self.document_store.index[document.id]["chunk_count"] = len(chunks)
                self.document_store._save_index()
    ...
    def query(self, question: str) -> RAGResponse:
        ...
        if self.vector_store.count() == 0:
            from nexusrag.agents.orchestrator import RAGResponse, ReasoningStep
            from nexusrag.agents.retriever_agent import RetrievalQuality

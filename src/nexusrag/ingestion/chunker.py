...
class HierarchicalChunker:
    ...
    def chunk(self, document: ParsedDocument) -> list[Chunk]:
        ...
        """
        # Get document name (prefer original filename)
        doc_name = (
            document.metadata.get("original_filename")
            or document.metadata.get("filename")
            or document.metadata.get("display_name")
            or "Unknown Document"
        )

        # Skip temp filenames
        if doc_name.startswith("tmp"):
            doc_name = document.metadata.get("display_name", "Document")

...
class HybridRetriever:
    ...
    def _apply_keyword_boost(
        ...
        for result in results:
            ...
            # Calculate boost (up to 15% boost for matching keywords)
...
class AdaptiveHybridRetriever(HybridRetriever):
    ...
    def _adapt_weights(self, query: str) -> None:
        ...
        if word_count <= 3 or has_technical:
            self.sparse_weight = min(0.6, self.base_sparse_weight + 0.2)
            self.dense_weight = 1 - self.sparse_weight
            logger.debug(f"Adapted weights: sparse-heavy ({self.sparse_weight})")

        # Long natural language queries favor dense (semantic)
        elif word_count >= 10:
            self.dense_weight = min(0.8, self.base_dense_weight + 0.2)
            self.sparse_weight = 1 - self.dense_weight
            logger.debug(f"Adapted weights: dense-heavy ({self.dense_weight})")

        # Default balanced
        else:
            self.dense_weight = self.base_dense_weight
            self.sparse_weight = self.base_sparse_weight
